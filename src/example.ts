import { SubmitSeedOffer, SubmitSeedRequest, collections, AzureUserToken } from './index';
...
console.log(`   - Filled requests: ${aliceOffer.filledRequests.length}`);
...
console.log(`   - From user: ${bobRequest1.fill?.offerUserId}\n`);
...
console.log(`   - From user: ${charlieRequest.fill?.offerUserId}\n`);
...
console.log(`   - Filled requests: ${charlieOffer.filledRequests.length}`);
console.log(`   - Filled for user: ${charlieOffer.filledRequests[0]?.requestUserId}`);
console.log(`   - Remaining offer: ${charlieOffer.remainingOffer?.quantity} packets\n`);

// Show final state
console.log('=== Final State ===');
console.log(`Total fills recorded: ${collections.getAllFills().length}`);
